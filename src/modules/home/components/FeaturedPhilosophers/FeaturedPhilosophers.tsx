...
const FeaturedPhilosophers: React.FC = () => {
  ...
  const fetchFeaturedPhilosophers = async (): Promise<Philosopher[]> => {
  ...
  const { data: philosophers = [] } = useQuery({
    queryKey: ['featuredPhilosophers'],
    queryFn: fetchFeaturedPhilosophers,
  });
            ...
            <Grid size={{ xs: 12, md: 4 }} key={philosopher.id}>
              <Card className="philosopher-preview-card" elevation={2}>
                ...
                <CardContent className="philosopher-preview-card__content">
                  <Typography variant="h5" className="philosopher-preview-card__name">
                    {lang === 'ar' ? philosopher.name_ar : philosopher.name_en}
                  </Typography>
                  <Typography variant="body2" className="philosopher-preview-card__dates">
                    {philosopher.birth} - {philosopher.death}
                  </Typography>
                  <Typography variant="body1" className="philosopher-preview-card__description">
                    {lang === 'ar' 
                ...
                <CardActions className="philosopher-preview-card__actions">
                  <Button
                    size="small"
                    onClick={() => 
                      navigate(`/philosophers/${philosopher.id}`, { state: { philosopher } })
                    }
